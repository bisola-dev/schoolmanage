...
$exmid=($_GET['exmid']);
$subjlame1=($_GET['subjlame']);
$exmlame1=($_GET['exmlame']);
$subjid=($_GET['subjid']);
$topid=($_GET['id']);
...
$sql = "DELETE FROM st_subtopicz WHERE id  IN ($crease)";

 if($conn->query($sql) === TRUE){ 
  echo $mes='<script type="text/javascript">
        alert("subtopic successfully deleted.");
        </script>';
         $ms = "subtopic successfully deleted. ";
...
//if (isset($_POST['ADD'])){
    //$exmid = trim(strip_tags($_POST['crease']));
    //$exmlame1 = trim(strip_tags($_POST['exmlame1']));


if (isset($_POST['reg'])){
   $exmid=trim(strip_tags($_POST['exmid']));
   $sublame = trim(strip_tags($_POST['sublame']));
   $subtoprof = trim(strip_tags($_POST['subtoprof']));

//echo $exmid;
$query2 = mysqli_query($conn, "SELECT * FROM st_subtopicz WHERE sublame='$sublame' AND exmid= $exmid" );
$num_rows = mysqli_num_rows($query2);

  if($sublame == ""||$subtoprof=="" ) {
  $mes="<script type ='text/javascript'>alert('please do not submit empty form.')
...
echo $session_error = "<script type ='text/javascript'>
                     alert('this subtopictype  has already been saved');
 ...
 $new22 = "INSERT INTO st_subtopicz (exmid,subjid,topid,subtoprof,sublame) VALUES('$exmid','$subjid','$topid','$subtoprof','$sublame')";

if (mysqli_query($conn, $new22)) {
 echo $mes="<script type='text/javascript'>
        alert('subtopic successfully saved.');
        
        </script>";

         $ms = "subtopic successfully saved. ";
  ...
  echo $mes='<script type="text/javascript">
        alert("topic  unadded please retry.");
        </script>';
         $ms = "topic  unadded please retry. ";
...
//if ($exmid=="") {header("location:addexam.php");
   # code...
//}
...
<head>
         ...
         <div class="content-wrapper">
            ...
            <section class="content-header">
               ...
               <div class="header-title">
                  <h1>ADD "<?php echo $exmlame1.'  '.$subjlame1;?>" SUBJECT SUB-TOPIC .</h1>
                  <small>Add and Delete "<?php echo $subjlame1;?>" Subject sub-Topic.</small>
               </div>
            </section>
            <!-- Main content -->
                        ...
                        <div class="panel-body">
                           ...
                           <form method="post" action="">
                            <div class="form-group ">
                                ...
                                <input type="hidden" id="username" class="form-control" name="exmid" value="<?php echo $exmid?>">

                                    <input type="text"  id="username" class="form-control" name="sublame" >
                                  
                                </div>

                                <label>Add Subtopic Profile </label>
                                 <textarea id="username" class="form-control" name="subtoprof" >
                                       </textarea>
 ...
 <?php 
            $bisola = mysqli_query($conn, "SELECT * FROM st_subtopicz WHERE exmid = $exmid ORDER BY sublame ASC");                          $count = 1;
                                    while ($row = mysqli_fetch_assoc($bisola)) {
                                       $exmid = $row['id'];
                                       $sublame1 = $row['sublame'];
                                       $subtoprof = $row['subtoprof'];
                                       $link='addsubtopic.php?id='.$exmid.'&sublame='.$sublame1.'&exmlame='.$exmlame1.'&exmid='.$subjid.'&subjlame='.$subjlame1;
                                    ?>
                                       
                                    <tr>
                                       <td><?php echo $count;?></td>
                                       <td><?php echo $sublame1;?></td>
          <td><button type="text"><a href="<?php echo $link; ?>">Addlessons</a></button></td>

                                      
                                       <td><form method="POST" action="" >
<input type="hidden"  value= "<?php echo $subjid;?>" name="crease">
