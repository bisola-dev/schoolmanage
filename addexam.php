...
$sql = "DELETE FROM st_exm WHERE id  IN ($crease)";
...
if (isset($_POST['reg'])){
   $exmlame = trim(strip_tags($_POST['exmlame']));
   $exmprof = trim(strip_tags($_POST['exmprof']));

$query2 = mysqli_query($conn, "SELECT * FROM st_exm WHERE exmlame='$exmlame'");
$num_rows = mysqli_num_rows($query2);

  if($exmlame == ""||$exmprof=="" ) {
  echo $mes="<script type ='text/javascript'>alert('please do not submit empty form.')
...
echo $session_error = "<script type ='text/javascript'>
 ...
 $new22 = "INSERT INTO st_exm (exmlame,exmprof) VALUES('$exmlame','$exmprof')";

if (mysqli_query($conn, $new22)) {
   echo $mes='<script type="text/javascript">
        alert("Exam successfully  saved.");
        </script>';
         $ms = "Exam successfully saved. ";

 

      $seexmlame = $_SESSION['exmlame'] = $exmlame;
  ...
  echo $mes='<script type="text/javascript">
        alert("Exam unadded please retry.");
        </script>';
         $ms = "Exam unadded please retry. ";
...
<head>
         ...
         <div class="content-wrapper">
            ...
            <section class="content-header">
               ...
               <div class="header-title">
                  <h1>ADD EXAM TYPE</h1>
                  <small>Add and Delete Examtype.</small>
               </div>
            </section>
            <!-- Main content -->
                           ...
                           <form method="post" action="">
                            <div class="form-group ">
                                <label>Add Exam Type </label>
                                    <input type="text"  id="username" class="form-control" name="exmlame" >
                                  
                                </div>

                                <label>Add Exam Profile </label>
                                    <textarea id="username" class="form-control" name="exmprof" >
                                       </textarea>
 ...
 <?php 
            ...
            $bisola = mysqli_query($conn, "SELECT * FROM st_exm ORDER BY exmlame ASC");
                                      $count = 1;
                                    while ($row = mysqli_fetch_assoc($bisola)) {
                                       $exmid = $row['id'];
                                       $exmlame1 = $row['exmlame'];
                                       $exmprof = $row['exmprof'];
                                       $link='addsubjectz.php?id='.$exmid.'&exmlame='.$exmlame1;
                                    ?>
                                       
                                    <tr>
                                       <td><?php echo $count;?></td>
                                       <td><?php echo $exmlame1;?></td>
                                      

      <td><button type="text"><a href="<?php echo $link; ?>">Add Subject</a></button></td>
