...
if (isset($_POST['reg'])){
   ...
   $subjlame = trim(strip_tags($_POST['subjlame']));
   $subjprof = trim(strip_tags($_POST['subjprof']));

//echo $exmid;
$query2 = mysqli_query($conn, "SELECT * FROM st_subj WHERE subjlame='$subjlame' AND exmid= $exmid" );
$num_rows = mysqli_num_rows($query2);

  if($subjlame == ""||$subjprof=="" ) {
...
echo $session_error = "<script type ='text/javascript'>
                     alert('this examtype  has already been saved');
                      window.location.href='addexam.php';
                        </script>";}
 ...
 $new22 = "INSERT INTO st_subj (exmid,subjlame,subjprof) VALUES('$exmid','$subjlame','$subjprof')";
...
<head>
         ...
         <div class="content-wrapper">
            ...
            <section class="content-header">
               ...
               <div class="header-title">
                  <h1>ADD <?php echo $exmlame1;?> SUBJECT TYPE</h1>
                  <small>Add and Delete <?php echo $exmlame1;?> Subject.</small>
               </div>
            </section>
            <!-- Main content -->
                           ...
                           <form method="post" action="">
                            <div class="form-group ">
                                ...
                                <input type="hidden" id="username" class="form-control" name="exmid" value="<?php echo $exmid?>">
                                    ...
                                    <input type="text"  id="username" class="form-control" name="subjlame" >
                                  
                                </div>

                                <label>Add Subject Profile </label>
                                 <textarea id="username" class="form-control" name="subjprof" >
                                       </textarea>
 ...
 <?php 
            $bisola = mysqli_query($conn, "SELECT * FROM st_subj WHERE exmid = $exmid ORDER BY subjlame ASC");                          $count = 1;
                                    while ($row = mysqli_fetch_assoc($bisola)) {
                                       $exmid = $row['exmid'];
                                       $subjid=$row['id'];
                                       $subjlame1 = $row['subjlame'];
                                       $subprof = $row['subprof'];
                                       $link='addtopic.php?exmid='.$exmid.'&subjlame='.$subjlame1.'&exmlame='.$exmlame1.'&id='.$subjid;
                                    ?>
                                       
                                    <tr>
                                       <td><?php echo $count;?></td>
                                       <td><?php echo $subjlame1;?></td>


         <td><button type="text"><a href="<?php echo $link; ?>">Add topic</a></button></td>
...
<p><input type="submit" name="delete" value="delete course"> </p>
         ...
         <footer class="main-footer">
            <strong>Copyright &copy; 2020 <a href="#">thememinister</a>.</strong> All rights reserved.
         </footer>
      </div>
      <!-- ./wrapper -->
      <!-- Start Core Plugins
